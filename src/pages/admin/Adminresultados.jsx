...
firma:  orden?.admin_firma_url || orden?.admin_firma || user.firma_digital || null,
  ...
  // ── FIX: exámenes en estado "Devuelto" (ya devueltos al especialista en
  // una revisión anterior) NO se pueden volver a validar/devolver desde
  // aquí hasta que el especialista los reenvíe (eso cambia su
  // estado_resultado en el backend). Por eso se excluyen del cálculo de
  ...
  const totalDevueltos  = examenesAccionables.filter(ex => decisiones[ex.key] === "devuelto").length;
    ...
    <Overlay onClose={!saving ? onClose : undefined}>
      ...
      <div style={{ ...modalInner, width: pdfPreview ? "min(1100px, 96vw)" : "760px", maxHeight: "none", height: "auto", display: "flex", flexDirection: "column", transition: "width 0.3s ease", overflow: "visible", margin: "auto" }}>
        ...
        <div style={{ padding: "1.1rem 1.25rem", display: "flex", flexDirection: "column", gap: "0.9rem" }}>
          ...
          {porEspecialista.map(({ ri, especialista, especialidad, estado_resultado, examenes }) => {
            // Grupo completo de este especialista ya devuelto en una
            // revisión anterior: todos sus exámenes muestran el aviso de
            // "esperando corrección" en vez de los botones de acción.
            const especialistaDevuelto = estado_resultado === "Devuelto";
            return (
                  ...
                  <div key={ex.key} style={{ borderBottom: "1px solid #F1F5F9", background: bgColor, transition: "background 0.3s", borderLeft: `3px solid ${borderColor}` }}>
                    ...
                    <div style={{ padding: "0.65rem 1rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem" }}>
                      ...
                      <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0, alignItems: "center" }}>
                        {especialistaDevuelto ? (
                          // ── FIX: examen ya devuelto en una revisión anterior — no se
                          // puede volver a validar ni devolver hasta que el especialista
                          // lo corrija y reenvíe (eso actualiza estado_resultado en el
                          // backend y, al recargar la orden, este aviso desaparece solo).
