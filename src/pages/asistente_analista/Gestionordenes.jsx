...
const PUEDE_REGENERAR_QR = ["Generada"]; // El QR solo tiene sentido mientras la orden no ha sido pagada

// ── Decodifica el payload de un JWT SIN verificar la firma ────────────────────
// Solo se usa para leer el "exp" y mostrar en la UI si el QR ya venció.
// La validación real de expiración/firma siempre ocurre en el backend.
...
export default function GestionOrdenes() {
  ...
  const handleRegistroRapido = async () => {
    ...
    try {
      ...
      setCredencialesGeneradas({ username, password, nombres, apellidos, correo });
  ...
  return (
    <div style={{ padding: "1.25rem", fontFamily: FONT, color: DARK }}>
                  ...
                  <div style={{ marginTop: "0.6rem" }}>
                    {credencialesGeneradas ? (
                      <div style={{ background: "linear-gradient(135deg, #F0FDF4, #DCFCE7)", border: "1.5px solid #86EFAC", borderRadius: "10px", padding: "0.9rem" }}>
                        ...
                        <p style={{ margin: "0 0 0.5rem", fontSize: "0.75rem", color: "#166534" }}>El paciente podrá cambiar esta contraseña luego desde su perfil.</p>
                        <p style={{ margin: "0 0 0.6rem", fontSize: "0.75rem", color: "#166534" }}>
                        ...
                        <div style={{ display: "flex", gap: "0.5rem" }}>
                          <button
                            ...
                            style={{ ...s.btnSecondary, flex: 1, padding: "0.6rem", fontSize: "0.85rem", textAlign: "center" }}
                          >
                            🖨️ Imprimir
                          </button>
                          <button type="button" onClick={resetFormRegistro} style={{ ...s.btnFull, flex: 1, padding: "0.6rem", fontSize: "0.85rem" }}>
      ...
      ════════════════════════════════════════════════════════════ */}
      {showDetalle && (
        <Overlay onClose={cerrarDetalle}>
          <div style={s.modalContainer}>
            <ModalHeader title="DETALLE" titleOrange="ORDEN" subtitle={showDetalle.numero_ticket || `Orden #${showDetalle.id_orden}`} onClose={cerrarDetalle} />
            <div style={s.modalBody}>
              {loadingDetalle ? (
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", gap: "0.65rem", flexWrap: "wrap" }}>
                      {PUEDE_EDITAR.includes(o.estado) && (
                        <button onClick={() => { cerrarDetalle(); abrirEditar(o); }} style={{ ...s.btnFull, flex: 1, background: "#3B82F6" }}>
                          ✏️ EDITAR ORDEN
                        </button>
                      )}
                      <button onClick={cerrarDetalle} style={s.btnCancel}>Cerrar</button>
                    </div>
                  </>
                );
              })()}
