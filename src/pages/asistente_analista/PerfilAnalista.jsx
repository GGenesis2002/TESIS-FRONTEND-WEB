...
export default function PerfilAnalista() {
  ...
  const cargar = async () => {
    setLoading(true);
    try {
      // A diferencia del especialista, el analista no necesita cargar asignaciones de exámenes
      const { data: p } = await API.get(`/personal/${userLocal.id}`);
      setPerfil(p);
      setInfoForm({
        nombres:   p.nombres   || "",
  ...
  const handleGuardarInfo = async () => {
    ...
    try {
      await API.put(`/personal/${userLocal.id}`, {
        ...
        // El atributo clave del analista es el turno, no la especialidad
    ...
    <>
      ...
      <div style={S.page}>
        ...
        <div style={{ marginBottom: "2rem" }}>
          ...
          <p style={S.sub}>Información de cuenta y datos de contacto</p>
        </div>
        ...
        <div style={S.grid}>
          ...
          {/* ══════ COLUMNA IZQUIERDA — Avatar + Turno ══════ */}
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>

            {/* Avatar card */}
            <div style={S.avatarCard}>
              <div style={S.avatarCircle}>
                {(userLocal.nombres || perfil?.nombres || "A")[0].toUpperCase()}
              </div>
              <h3 style={S.avatarName}>
                {perfil?.nombres || userLocal.nombres} {perfil?.apellidos || userLocal.apellidos}
              </h3>
              <span style={S.rolBadge}>ASISTENTE / ANALISTA</span>

              {/* Mostrar el turno en lugar de la especialidad */}
              {perfil?.turno && (
                <div style={S.turnoBadge}>
                  🕒 Turno: {perfil.turno}
                </div>
              )}
              ...
              <div style={S.twoCol}>
                {editandoInfo ? (
                  <>
                    ...
                    <InfoField label="Turno"  value={perfil?.turno} />
...
const S = {
  ...
  accent:     { color: "#3B82F6" }, // Cambiado a azul para diferenciar del especialista (morado)
  ...
  avatarCircle: { width: "80px", height: "80px", borderRadius: "50%", background: "linear-gradient(135deg, #3B82F6, #60A5FA)", color: "#FFF", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: "2rem", marginBottom: "1rem" },
  ...
  rolBadge:     { fontFamily: "'Barlow Condensed', sans-serif", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.15em", color: "#3B82F6", background: "rgba(59,130,246,0.1)", padding: "0.3rem 0.75rem", borderRadius: "20px", border: "1px solid rgba(59,130,246,0.2)" },
  turnoBadge:   { marginTop: "0.6rem", background: "rgba(59,130,246,0.06)", border: "1px dashed rgba(59,130,246,0.25)", borderRadius: "8px", padding: "0.4rem 0.75rem", fontSize: "0.78rem", color: "#2563EB", fontFamily: "'Barlow', sans-serif", fontWeight: 500 },
  ...
  toggleBtn:  { background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)", color: "#3B82F6", padding: "0.45rem 0.9rem", borderRadius: "8px", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: "0.77rem", cursor: "pointer" },
  ...
  spinner:    { width: "28px", height: "28px", border: "3px solid #F1F5F9", borderTop: "3px solid #3B82F6", borderRadius: "50%", animation: "spin 0.7s linear infinite" },
};